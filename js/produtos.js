...
/* ==========================================
   ...
   Responsável por: montar cards do catálogo, modal de detalhes,
   navegação dos carrosséis de imagens
   ========================================== */

// Estado do carousel do modal
...
function getImagens(p) {
    ...
    if (p.imagens && Array.isArray(p.imagens)) p.imagens.forEach(i => { if (i && !imgs.includes(i)) imgs.push(i); });
    return imgs.length > 0 ? imgs : ['https://via.placeholder.com/400x300/cccccc/666666?text=Sem+Imagem'];
}
...
function renderTodosProdutos() {
    ...
    if (produtos.length === 0) {
        grade.innerHTML = '<div class="empty-message" style="display:block; grid-column:1/-1;"><i class="fas fa-box-open" style="font-size:2rem; margin-bottom:var(--spacing-md); display:block; opacity:0.4;"></i><p style="margin:0;">Nenhum produto cadastrado nesta loja ainda.</p></div>';
        return;
    }
    ...
    produtos.forEach((p, idx) => {
        ...
        let carouselHtml = '';
        if (imgs.length > 1) {
            const trackImgs = imgs.map(src => `<img loading="lazy" src="${escapeHTML(src)}" onerror="this.src='https://via.placeholder.com/400x300/cccccc/666666?text=Sem+Imagem'">`).join('');
            const dots = imgs.map((_, i) => `<span class="card-img-dot${i === 0 ? ' active' : ''}" data-idx="${i}"></span>`).join('');
            carouselHtml = `<div class="card-img-carousel" data-current="0" data-total="${imgs.length}"><div class="card-img-carousel-track">${trackImgs}</div><button class="card-img-nav prev" onclick="event.stopPropagation(); cardCarouselNav(this, -1)"><i class="fas fa-chevron-left"></i></button><button class="card-img-nav next" onclick="event.stopPropagation(); cardCarouselNav(this, 1)"><i class="fas fa-chevron-right"></i></button><div class="card-img-dots">${dots}</div></div>`;
        } else {
            carouselHtml = `<div class="card-img-carousel"><div class="card-img-carousel-track"><img loading="lazy" src="${escapeHTML(imgs[0])}" onerror="this.src='https://via.placeholder.com/400x300/cccccc/666666?text=Sem+Imagem'"></div></div>`;
        }
        card.innerHTML = `<span class="tag-tipo" style="background:${c.cor}">${escapeHTML(c.nome)}</span>${p.fixo ? '' : '<span class="tag-novo">NOVO</span>'}${carouselHtml}<div class="card-info"><div class="card-nome">${escapeHTML(p.nome)}</div><div class="card-preco">${formatarPreco(p.preco)}<span class="card-btn-add"><i class="fas fa-plus"></i></span></div></div>`;
        card.addEventListener('click', () => abrirModalDetalhes(p));
        card.querySelector('.card-btn-add').addEventListener('click', e => { e.stopPropagation(); adicionarAoCarrinho(p); });
...
function cardCarouselNav(btn, dir) {
    ...
    const track = carousel.querySelector('.card-img-carousel-track');
...
function abrirModalDetalhes(p) {
    ...
    track.innerHTML = imgs.map(src => `<img src="${escapeHTML(src)}" onerror="this.src='https://via.placeholder.com/400x400/cccccc/666666?text=Sem+Imagem'">`).join('');
    track.style.transform = 'translateX(0)';
    const dots = document.getElementById('modal-dots');
    if (imgs.length > 1) {
        dots.innerHTML = imgs.map((_, i) => `<span class="modal-img-dot${i === 0 ? ' active' : ''}" onclick="modalCarouselGoTo(${i})"></span>`).join('');
...
function modalCarouselUpdate() {
    ...
    document.querySelectorAll('#modal-dots .modal-img-dot').forEach((d, i) => d.classList.toggle('active', i === modalCarouselCurrent));
}

// Suporte a swipe touch no carousel do modal
(function() {
    let startX = 0;
